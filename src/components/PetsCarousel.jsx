...
// Вспомогательная функция для форматирования возраста (перенесена из Pets.jsx)
...
// ── Обновленная карточка питомца в карусели (в стиле каталога) ────────
function CarouselCard({ pet, user, isFavoriteInit }) {
  ...
  // ... (весь return карточки остается вашим)
    ...
    <Link to={`/pets/${pet.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
      ...
      >
        ...
        <div style={{ padding: '14px 16px', flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          ...
          {/* Блок "Подробнее" убран, как вы и просили */}
        </div>
      </div>
    </Link>
  );
